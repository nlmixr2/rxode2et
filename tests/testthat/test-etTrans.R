...
")
  ...
  mod <- rxode2parse::rxode2parse("    CO = (187 * WT^0.81) * 60/1000
    QHT = 4 * CO/100
    QBR = 12 * CO/100
    QMU = 17 * CO/100
    QAD = 5 * CO/100
  ...
  mod <- rxode2parse::rxode2parse("    x1(0) = x10\n    d/dt(x1) = a * x1\n    Volume = x1;\ncmt(Volume);\n\n    nlmixr_pred <- Volume")
...
")
  ...
  test_that("etTrans", {
    ...
    t0 <- suppressWarnings(etTrans(events2,
                                   rxode2parse::rxode2parse(lst$object), FALSE, FALSE, FALSE, FALSE, NULL, character(0)))
    expect_s3_class(t0, "rxEtTran")

    t1 <- etTrans(events2, rxode2parse::rxode2parse(lst$object), FALSE, FALSE, FALSE, TRUE, NULL, character(0))
